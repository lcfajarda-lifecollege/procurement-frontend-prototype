...
function ProcurementReview({ request, onBack, onComplete, onNotify }: { request: PurchaseRequest; onBack: () => void; onComplete: () => void; onNotify: (message: string) => void }) {
  ...
  const [notes, setNotes] = useState('');
  const checks = ['Request details are complete', 'Product specifications are clear', 'Quantities and units of measure are reasonable', 'Required date is achievable', 'Vendor sourcing is required', 'Supporting documents are available'];
