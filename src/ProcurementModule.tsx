...
function purchaseOrderTotal(request: PurchaseRequest) {
  ...
  return selectedQuote ? rfqQuoteTotal(request, selectedQuote) : request.amount;
}
...
export default function ProcurementModule({ activePath, sessionUser, onNavigate, previewRole, onWorkItemsChange }: { activePath: string; sessionUser: SessionUser; onNavigate: (path: string) => void; previewRole: string; onWorkItemsChange: (items: ProcurementWorkItemSummary[]) => void }) {
  ...
  async function runAction(id: string, action: string) {
    ...
    setRequests((current) => current.map((item) => {
      ...
      if (action === 'approve') nextStatus = item.status === 'For DT Approval' ? 'For Requester Selection' : item.status === 'For Department Approval' ? purchaseOrderTotal(item) <= 100000 ? 'For Finance Approval' : purchaseOrderTotal(item) <= 999999 ? 'For COO Approval' : 'For President Approval' : ['For Finance Approval', 'For COO Approval', 'For President Approval'].includes(item.status) ? 'PO Approved' : 'For Procurement Review';
...
function RoutingApprovalDetail({ request, onApprove, onNotify, onOpenDtReview }: { request: PurchaseRequest; onApprove: (id: string) => void; onNotify: (message: string) => void; onOpenDtReview: () => void }) {
  ...
  const approvalAmount = isPoApproval ? purchaseOrderTotal(request) : request.amount;
  ...
  const next = isDt ? 'Requester quotation selection' : isDepartment ? approvalAmount <= 100000 ? 'Finance approval' : approvalAmount <= 999999 ? 'COO approval' : 'President approval' : 'Approved and ready for issue';
  const button = isDt ? 'Complete DT review' : isDepartment ? approvalAmount <= 100000 ? 'Approve to Finance' : approvalAmount <= 999999 ? 'Approve to COO' : 'Approve to President' : 'Approve Purchase Order';
  return <section className="proc-card review-card">
    <div className="review-header"><div><span className="status-pill warning">{request.status}</span><h3>{request.title}</h3><p>{documentId} · {isPoApproval ? `Source ${request.id}` : `Requested by ${request.requester}`}</p></div><div className="review-estimate-reference"><span>{isPoApproval ? 'Purchase Order total' : 'Request estimate'}</span><strong>{money.format(approvalAmount)}</strong></div></div>
