...
interface PurchaseRequest {
  id: string;
  title: string;
  purposeType?: 'Activity' | 'Event';
  purpose?: string;
  invoices?: RecordAttachment[];
  department: string;
  amount: number;
  category: string;
  requester: string;
...
function purchaseOrderTotal(request: PurchaseRequest) {
  const selectedQuote = request.rfqQuotes?.find((quote) => quote.vendorName === request.vendorName);
  const requestItems = request.items ?? [];
  if (!selectedQuote || selectedQuote.status !== 'Responded' || !requestItems.length || requestItems.some((item) => !selectedQuote.items.some((quotedItem) => quotedItem.name === item.name))) return null;
  return rfqQuoteTotal(request, selectedQuote);
}

function purchaseOrderTotalLabel(request: PurchaseRequest) {
  const total = purchaseOrderTotal(request);
  return total === null ? 'PO total pending' : money.format(total);
}

function workflowFinancialAmount(request: PurchaseRequest) {
  return isPurchaseOrderStatus(request.status) ? purchaseOrderTotal(request) : request.amount;
}
...
const purchaseOrderWorkflowStages = [
  ...
  { name: 'Closed', detail: 'The Automated Payment System admin closes the order in APS. Procurement reflects the confirmed closure.' },
];
...
export default function ProcurementModule({ activePath, sessionUser, onNavigate, previewRole, onWorkItemsChange }: { activePath: string; sessionUser: SessionUser; onNavigate: (path: string) => void; previewRole: string; onWorkItemsChange: (items: ProcurementWorkItemSummary[]) => void }) {
  ...
  async function runAction(id: string, action: string) {
    ...
    setRequests((current) => current.map((item) => {
      if (item.id !== id) return item;
      if (action === 'approve') {
        const poTotal = purchaseOrderTotal(item);
        nextStatus = item.status === 'For DT Approval' ? 'For Requester Selection' : item.status === 'For Department Approval' ? poTotal === null ? null : poTotal <= 100000 ? 'For Finance Approval' : poTotal <= 999999 ? 'For COO Approval' : 'For President Approval' : ['For Finance Approval', 'For COO Approval', 'For President Approval'].includes(item.status) ? poTotal === null ? null : 'PO Approved' : 'For Procurement Review';
      }
  ...
  const currentView = useMemo(() => {
    ...
    if (dtQuotationReviewMatch) {
      ...
      return <DtQuotationComparisonPage request={request} onBack={() => onNavigate('/approvals')} onComplete={(notes) => { const createdAt = new Date().toISOString(); const reviewNotes = notes.trim() || 'Digital Transformation confirmed that technically suitable vendor offers are available for every technology product.'; setRequests((current) => current.map((item) => item.id === request.id ? { ...item, dtReviewNotes: reviewNotes, dtReviewedAt: createdAt, dtReviewedBy: sessionUser?.name || 'Digital Transformation Team', updatedAt: createdAt, history: [...(item.history ?? []), { action: 'complete_dt_review', actor: sessionUser?.email || 'dt.review@life.edu.ph', detail: reviewNotes, createdAt }] } : item)); void runAction(request.id, 'approve'); onNavigate('/approvals'); }} onNotify={notify} />;
    }
    ...
    if (activePath === '/receiving') return <ReceivingQueueView requests={requests.filter((item) => ['PO Acknowledged','Partially Received','Received','Paid','Filed'].includes(item.status))} role={role} onAction={runAction} onRequestSaved={(updated) => setRequests((current) => current.map((item) => item.id === updated.id ? updated : item))} />;
...
function RequestsView({ requests, role, requesterName, onNavigate, onNew }: { requests: PurchaseRequest[]; role: Role; requesterName: string; onNavigate: (path: string) => void; onNew: () => void }) {
  ...
  const [filter, setFilter] = useState<'all' | 'department' | 'action'>('all');
  ...
  useEffect(() => { if (role === 'Requester') setFilter('all'); }, [role]);
  ...
  return <div className="proc-page">
    ...
    {selected ? <div className="approval-layout request-master-detail">
      <section className="proc-card queue-card">
        ...
        {filtered.map((request) => <button type="button" className={`queue-item ${selected.id === request.id ? 'selected' : ''}`} key={request.id} onClick={() => setSelectedId(request.id)}><span className="queue-badge">{request.requester.split(' ').map((part) => part[0]).join('').slice(0,2)}</span><span><b>{request.title}</b><small>{request.id} · {request.status}</small>{role === 'Requester' && request.status === 'For Requester Selection' ? <em className="queue-action-label">Choose vendor</em> : null}</span></button>)}
      </section>
      <section className="proc-card review-card request-detail-card">
        <div className="review-header"><div><StatusBadge>{selected.status}</StatusBadge><h3>{selected.title}</h3><p><strong className="inline-pr-number">{selected.id}</strong> · Requested by {selected.requester}</p></div><div className="review-estimate-reference"><span>Estimated cost</span><strong>{money.format(selected.amount)}</strong></div></div>
        <RequestPurpose request={selected} /><div className="detail-grid"><Detail label="Department" value={selected.department} /><Detail label="Required date" value={selected.due} /><Detail label="Product lines" value={String(selected.items?.length ?? 0)} /><Detail label="Total quantity" value={String(requestTotalQuantity(selected))} /></div>
...
function RequestPurpose({ request }: { request: PurchaseRequest }) {
  return <div className="request-purpose"><small>Purpose{request.purposeType ? ` / ${request.purposeType}` : ''}</small><p>{request.purpose || 'Not recorded'}</p></div>;
}

function DtReviewGuidance({ request }: { request: PurchaseRequest }) {
  return <div className="request-purpose"><small>DT technical review</small><p>{request.dtReviewNotes || 'No review notes recorded'}</p>{request.dtReviewedBy && request.dtReviewedAt ? <p>{request.dtReviewedBy} · {formatMovementTime(request.dtReviewedAt)}</p> : <p>Approval not recorded</p>}</div>;
}

function ApprovalItems({ request }: { request: PurchaseRequest }) {
  const quote = request.rfqQuotes?.find((item) => item.vendorName === request.vendorName);
  return <section className="approval-products"><h4>Requested products</h4><RequestPurpose request={request} /><div className="approval-products-scroll"><table><thead><tr><th>Item</th><th>Description / Specifications</th><th>Category</th><th>Quantity</th><th>UOM</th><th>Selected unit price</th><th>Line total</th></tr></thead><tbody>{(request.items ?? []).map((item, index) => { const price = quote?.items.find((line) => line.name === item.name)?.unitPrice; return <tr key={index}><th scope="row">{item.name}</th><td>{itemDescription(item)}</td><td>{item.category}</td><td>{item.quantity}</td><td>{itemUom(item)}</td><td>{price !== undefined ? money.format(price) : 'Not selected'}</td><td>{price !== undefined ? money.format(price * item.quantity) : 'Not selected'}</td></tr>; })}</tbody></table></div><div className="detail-grid"><Detail label="Requester" value={request.requester} /><Detail label="Required date" value={request.due} /><Detail label="Selected vendor" value={request.vendorName || 'Not selected'} /><Detail label="Document Reference Number" value={quote?.reference || 'Not recorded'} /><Detail label="Payment terms" value={quote?.terms || 'Not recorded'} /><Detail label="Delivery" value={quote ? `${quote.deliveryDays} days` : 'Not recorded'} /></div>{requestIncludesTechnology(request) ? <DtReviewGuidance request={request} /> : null}</section>;
}
...
function RoutingApprovalDetail({ request, onApprove, onNotify, onOpenDtReview }: { request: PurchaseRequest; onApprove: (id: string) => void; onNotify: (message: string) => void; onOpenDtReview: () => void }) {
  ...
  const next = isDt ? 'Requester quotation selection' : isDepartment ? !approvalAmountAvailable ? 'Quotation selection required' : approvalAmount <= 100000 ? 'Finance approval' : approvalAmount <= 999999 ? 'COO approval' : 'President approval' : !approvalAmountAvailable ? 'Quotation selection required' : 'Approved and ready for issue';
  const button = isDt ? 'Complete DT review' : isDepartment ? !approvalAmountAvailable ? 'Approval unavailable' : approvalAmount <= 100000 ? 'Approve to Finance' : approvalAmount <= 999999 ? 'Approve to COO' : 'Approve to President' : !approvalAmountAvailable ? 'Approval unavailable' : 'Approve Purchase Order';
  return <section className="proc-card review-card">
    <div className="review-header"><div><span className="status-pill warning">{request.status}</span><h3>{request.title}</h3><p>{documentId} · {isPoApproval ? `Source ${request.id}` : `Requested by ${request.requester}`}</p></div><div className="review-estimate-reference"><span>{isPoApproval ? 'Purchase Order total' : 'Request estimate'}</span><strong>{isPoApproval ? purchaseOrderTotalLabel(request) : money.format(request.amount)}</strong></div></div>
    ...
    {isDt ? <RequestPurpose request={request} /> : null}
    {!isDt ? <ApprovalItems request={request} /> : null}
...
function MultiSourcingView({ requests, onAction, onVendorSaved, onNotify }: { requests: PurchaseRequest[]; onAction: (id: string, action: string) => void; onVendorSaved: (request: PurchaseRequest) => void; onNotify: (message: string) => void }) {
  ...
  const filtered = displayRequests.filter((item) => {
    const matchesQuery = `${sourcePurchaseRequestNumber(item)} ${item.title} ${item.vendorName ?? ''}`.toLowerCase().includes(query.toLowerCase());
  ...
  if (!displayRequests.length) return <EmptyWorkflow title="RFQ & Vendor Sourcing" detail="A submitted Purchase Request will appear after Procurement Review." />;
  return <div className="proc-page"><PageHeading eyebrow="Vendor sourcing" title="All RFQs" detail="Invite qualified vendors, record quotations, route technical items for review, and prepare the requester’s selection." /><section className="proc-filterbar"><div className="proc-search"><Search size={17} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search RFQ number, request, or vendor" /></div><button className={`filter-chip ${status === 'all' ? 'active' : ''}`} onClick={() => setStatus('all')}>All</button><button className={`filter-chip ${status === 'draft' ? 'active' : ''}`} onClick={() => setStatus('draft')}>Sourcing</button><button className={`filter-chip ${status === 'sent' ? 'active' : ''}`} onClick={() => setStatus('sent')}>Awaiting replies</button><button className={`filter-chip ${status === 'quotes' ? 'active' : ''}`} onClick={() => setStatus('quotes')}>Quotes received</button><button className={`filter-chip ${status === 'selection' ? 'active' : ''}`} onClick={() => setStatus('selection')}>Ready for PO</button></section><section className="proc-card rfq-list-card"><CardHeader title={`Sourcing records (${filtered.length})`} icon={FileText} />{filtered.map((request) => { const sourceNumber = sourcePurchaseRequestNumber(request); const documentNumber = request.status === 'For Procurement Review' ? sourceNumber : sourceNumber.replace('PR-', 'RFQ-'); return <button type="button" className={`rfq-list-row ${request.demoStagePreview ? 'stage-preview-row' : ''}`} key={request.id} onClick={() => { setSelectedId(request.id); window.scrollTo({ top: 0 }); }}><span className="rfq-list-icon"><FileText size={18} /></span><span><b>{documentNumber}</b><small>{request.title} · Source {sourceNumber}</small></span><span><StatusBadge>{request.status}</StatusBadge><small>{request.demoStagePreview ? 'Stage preview · Read only' : `${sourcingLots(request).length} sourcing lot${sourcingLots(request).length === 1 ? '' : 's'} · ${request.rfqQuotes?.length ?? 0} vendors`}</small></span><ArrowRight className="rfq-row-chevron" size={18} /></button>; })}{!filtered.length ? <div className="table-empty">No sourcing records match the current filters.</div> : null}</section></div>;
}
...
function SourcingWorkflowView({ request, onBack, onAction, onRequestSaved, onNotify }: { request: PurchaseRequest; onBack: () => void; onAction: (id: string, action: string) => void; onRequestSaved: (request: PurchaseRequest) => void; onNotify: (message: string) => void }) {
  const previewOnly = Boolean(request.demoStagePreview);
  const lots = sourcingLots(request);
  ...
  const canSimulateReceipt = lots.every((lot) => lot.items.length > 0 && quotes.filter((quote) => ['Sent', 'Viewed', 'Overdue', 'Responded'].includes(quote.status) && quoteSupportsLot(request, quote, lot)).length >= 2);
  ...
  const simulateQuotationReceipt = () => {
    if (previewOnly || !sent || !canSimulateReceipt) return;
    const createdAt = new Date().toISOString();
    const simulatedQuotes = quotes.map((quote, index) => ['Sent', 'Viewed', 'Overdue'].includes(quote.status)
      ? { ...quote, status: 'Responded' as const, reference: quote.reference.trim() || `DEMO-${request.id}-${index + 1}` }
      : quote);
    onRequestSaved({
      ...request,
      status: 'Quotations Received',
      rfqQuotes: simulatedQuotes,
      updatedAt: createdAt,
      history: [...(request.history ?? []), {
        action: 'record_quotations',
        actor: 'procurement@life.edu.ph',
        detail: 'Demo: vendor replies simulated using the current quotation values. Existing responses retained; declined and withdrawn invitations excluded.',
        createdAt,
      }],
    });
    onNotify('Demo quotations received. Submit quotations for review to continue.');
  };
  ...
  if (request.status === 'For Procurement Review') return <ProcurementReview request={request} previewOnly={previewOnly} onBack={onBack} onComplete={() => onAction(request.id, 'complete_review')} onNotify={onNotify} />;
  return <div className={`proc-page sourcing-workflow-page ${previewOnly ? 'stage-preview-mode' : ''}`}>
    <section className="rfq-commandbar"><div className="rfq-actions"><button className="proc-secondary" type="button" onClick={onBack}><ArrowLeft size={16} />Back to sourcing</button><button className="proc-secondary" type="button" onClick={() => setPreviewOpen(true)}><Eye size={16} />Preview {activeLot.category} form</button>{!previewOnly && draft ? <button className="proc-primary" type="button" disabled={!allLotsShortlisted} onClick={inviteVendors}><Send size={16} />Send all category RFQs</button> : null}{!previewOnly && sent ? <button className="proc-primary" type="button" disabled={!allLotsResponded} onClick={closeQuotations}><Check size={16} />Close all RFQs</button> : null}{!previewOnly && sent ? <button className="proc-secondary" type="button" disabled={!canSimulateReceipt} title="Simulate vendor replies using the current quotation values; declined and withdrawn invitations are excluded." onClick={simulateQuotationReceipt}><CheckCircle2 size={16} />Mark quotations received (Demo)</button> : null}{!previewOnly && quotationsReceived ? <button className="proc-primary" type="button" onClick={submitQuotations}><ArrowRight size={16} />Submit quotations for review</button> : null}{!previewOnly && readyForPo ? <button className="proc-primary" type="button" disabled={!allLotsAwarded} onClick={() => onAction(request.id, 'create_po')}><ShoppingCart size={16} />Create {lots.length} {lots.length === 1 ? 'PO' : 'POs'}</button> : null}</div><div className="rfq-status-track"><span className={draft ? 'active' : 'done'}>Vendor sourcing</span><span className={sent ? 'active' : ['Quotations Received','For DT Approval','For Requester Selection','Ready for PO Creation'].includes(request.status) ? 'done' : ''}>RFQs sent</span><span className={quotationsReceived ? 'active' : ['For DT Approval','For Requester Selection','Ready for PO Creation'].includes(request.status) ? 'done' : ''}>Quotations received</span><span className={readyForPo ? 'active' : ''}>Requester selection</span></div></section>
    {previewOnly ? <section className="stage-preview-notice"><Eye size={18} /><div><b>Read-only lifecycle preview</b><small>This example shows how {sourcePurchaseRequestNumber(request)} appears at the {request.status} stage. It does not change the live request.</small></div></section> : null}
    <section className="proc-card sourcing-record-header"><div><span className="proc-eyebrow">{sourcePurchaseRequestNumber(request).replace('PR-', 'RFQ-')} · Source {sourcePurchaseRequestNumber(request)}</span><h2>{request.title}</h2><p>{request.department} · Requested by {request.requester}</p></div><div><StatusBadge>{request.status}</StatusBadge><strong>{money.format(request.amount)} estimated</strong></div></section>
    ...
    {quotationsReceived ? <section className="proc-card quotation-validation-notes"><label className="notes-field"><span>Procurement Validation Notes</span><textarea disabled={previewOnly} value={validationNotes} onChange={(event) => setValidationNotes(event.target.value)} placeholder="Document missing requirements, quotation conditions, eligibility concerns, exclusions, or instructions for the next reviewer" /></label><div><ClipboardCheck size={18} /><span><b>Shared with the next reviewer</b><small>{requestIncludesTechnology(request) ? 'This note will be shown to Digital Transformation during technical review and later to the requester.' : 'This note will be shown directly to the requester before vendor selection.'}</small></span></div></section> : null}
...
function RfqListView({ request, onOpen }: { request: PurchaseRequest; onOpen: () => void }) {
  ...
  return <div className="proc-page"><PageHeading eyebrow="Vendor sourcing" title="All RFQs" detail="Invite vendors, receive quotations, compare offers, select the best vendor, and create the PO." /><section className="proc-filterbar"><div className="proc-search"><Search size={17} /><input placeholder="Search RFQ number, request, or vendor" /></div><button className="filter-chip active">All RFQs</button><button className="filter-chip">Draft</button><button className="filter-chip">Sent</button><button className="filter-chip">Needs reply</button></section><section className="proc-card rfq-list-card"><CardHeader title="Requests for quotation" icon={FileText} /><button type="button" className="rfq-list-row" onClick={onOpen}><span className="rfq-list-icon"><FileText size={18} /></span><span><b>{documentNumber}</b><small>{request.title} · Created from {request.id}</small></span><span><StatusBadge>{request.status}</StatusBadge><small>Multiple vendors</small></span><ArrowRight className="rfq-row-chevron" size={18} /></button></section></div>;
}

function ProcurementReview({ request, previewOnly = false, onBack, onComplete, onNotify }: { request: PurchaseRequest; previewOnly?: boolean; onBack: () => void; onComplete: () => void; onNotify: (message: string) => void }) {
  const [notes, setNotes] = useState('');
  return <div className="proc-page"><PageHeading eyebrow="Procurement Officer workspace" title="Procurement Review" detail="Validate the submitted Purchase Request before beginning vendor sourcing." action={<button className="proc-secondary" type="button" onClick={onBack}><ArrowLeft size={16} />Back to all RFQs &amp; Sourcing</button>} />{previewOnly ? <section className="stage-preview-notice"><Eye size={18} /><div><b>Read-only lifecycle preview</b><small>This example shows how {sourcePurchaseRequestNumber(request)} appears at the Procurement Review stage.</small></div></section> : null}<div className="approval-layout procurement-review-layout"><section className="proc-card queue-card"><CardHeader title="For procurement review" icon={Inbox} /><button type="button" className="queue-item selected"><span className="queue-badge">{request.requester.split(' ').map((part) => part[0]).join('').slice(0,2)}</span><span><b>{request.title}</b><small>{sourcePurchaseRequestNumber(request)} · Stakeholders notified</small></span></button></section><section className="proc-card review-card"><div className="review-header"><div><StatusBadge>{request.status}</StatusBadge><h3>{request.title}</h3><p>{sourcePurchaseRequestNumber(request)} · Requested by {request.requester}</p></div></div><div className="detail-grid"><Detail label="Department" value={request.department} /><Detail label="Required date" value={request.due} /><Detail label="Product lines" value={String(request.items?.length ?? 0)} /><Detail label="Total quantity" value={String(requestTotalQuantity(request))} /></div><div className="movement-history-title"><strong>Requested products</strong><span>Review specifications and quantities</span></div><div className="request-detail-items procurement-review-items">{request.items?.map((item, index) => <div key={`${item.name}-${index}`}><span><b>{item.name}</b><small>{item.category} · {itemUom(item)}</small><em>{itemDescription(item)}</em></span><span>{item.quantity} {itemUom(item)}</span></div>)}</div><label className="notes-field procurement-review-notes"><span>Procurement review notes</span><textarea disabled={previewOnly} value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Record clarifications, sourcing instructions, or vendor requirements" /></label>{!previewOnly ? <div className="action-row"><button className="proc-secondary" disabled={!notes.trim()} onClick={() => onNotify(`${request.id} returned to requester for clarification: ${notes.trim()}`)}>Return for clarification</button><button className="proc-primary" onClick={onComplete}><Check size={17} />Complete review and begin sourcing</button></div> : null}</section></div></div>;
}
...
function PurchaseOrdersView({ requests, role, onAction }: { requests: PurchaseRequest[]; role: Role; onAction: (id: string, action: string) => void }) {
  return <PurchaseOrdersDetailView requests={requests} role={role} onAction={onAction} />;
}
...
function PurchaseOrdersDetailView({ requests, role, onAction }: { requests: PurchaseRequest[]; role: Role; onAction: (id: string, action: string) => void }) {
  ...
  const poActivity = purchaseOrderActivity(request);
  const poTotal = purchaseOrderTotal(request);
  const poTotalAvailable = poTotal !== null;
  const nextStep = !poTotalAvailable ? { title: 'Quotation selection required', detail: 'Select a complete vendor quotation before submitting this Purchase Order for approval.' } : draft ? { title: 'Department Head Approval', detail: 'Submit the PO draft to the requesting department’s head for approval.' } : request.status === 'For Department Approval' ? { title: `${executiveNotificationRole(poTotal)} Approval`, detail: 'The Department Head must approve before the PO is routed by total amount.' } : ['For Finance Approval', 'For COO Approval', 'For President Approval'].includes(request.status) ? { title: 'Approved and Issued', detail: `${assignedTeamForStatus(request.status)} approval is required before Procurement can issue the PO.` } : approved ? { title: 'Email PO to Vendor', detail: 'All internal approvals are complete. Procurement may now issue the official PO.' } : awaitingAcknowledgement ? { title: 'Vendor Acknowledgement', detail: 'Record the vendor’s acceptance and expected delivery date.' } : ['PO Acknowledged', 'Partially Received'].includes(request.status) ? { title: 'Delivery and Receiving', detail: 'Monitor delivery, inspection, and department acceptance.' } : ['Received', 'Paid'].includes(request.status) ? { title: 'Closed', detail: 'Awaiting closure by the Automated Payment System admin. Procurement will reflect the confirmed APS closure.' } : { title: 'Closed', detail: 'The purchasing record is closed and retained for audit.' };
  return <div className="proc-page"><PageHeading eyebrow="External commitment" title="Purchase Orders" detail="Route draft orders for approval, issue approved POs, and monitor vendor acknowledgement." /><div className="approval-layout po-master-detail"><section className="proc-card queue-card"><CardHeader title={`Purchase orders (${requests.length})`} icon={ShoppingCart} />{requests.map((item) => <button type="button" className={`queue-item ${request.id === item.id ? 'selected' : ''}`} key={item.id} onClick={() => setSelectedId(item.id)}><span className="queue-badge">PO</span><span><b>{item.id.replace('PR-', 'PO-')}</b><small>{item.title} · {item.status}</small></span></button>)}</section><section className="proc-card po-card"><div className="po-top"><div><StatusBadge>{request.status}</StatusBadge><h3>{request.id.replace('PR-', 'PO-')}</h3><p>{request.title}</p><small>{request.vendorName || 'No vendor selected'}</small></div><div className={`po-total ${poTotalAvailable ? '' : 'pending'}`}><small>PO total</small><strong>{purchaseOrderTotalLabel(request)}</strong></div></div><div className="movement-history-title po-lifecycle-title"><strong>Purchase Order lifecycle</strong><span>Stage {currentPoStage + 1} of {purchaseOrderWorkflowStages.length}</span></div><div className="pr-journey po-record-lifecycle">{purchaseOrderWorkflowStages.map((stage, index) => <div className={`${index < currentPoStage ? 'complete' : ''} ${index === currentPoStage ? 'current' : ''}`} key={stage.name} title={stage.detail}><span>{index < currentPoStage ? <Check size={12} /> : index + 1}</span><small>{stage.name}</small></div>)}</div><div className={`po-next-step ${approvalPending ? 'approval' : ''}`}><span>{approvalPending ? <UserCheck size={18} /> : <ArrowRight size={18} />}</span><div><small>Next step</small><b>{nextStep.title}</b><p>{nextStep.detail}</p></div></div><div className="action-row"><button className="proc-secondary" disabled={!poTotalAvailable} onClick={() => setDocumentRequest(request)}><FileText size={16} />View PO PDF</button>{draft && canManagePo ? <button className="proc-primary" disabled={!poTotalAvailable} onClick={() => onAction(request.id, 'submit_po_department')}><UserCheck size={16} />Submit for Department Approval</button> : approved && canManagePo ? <button className="proc-primary" disabled={!poTotalAvailable} onClick={() => onAction(request.id, 'send_po')}><Mail size={16} />Email approved PO to vendor</button> : awaitingAcknowledgement && canManagePo ? <button className="proc-primary" disabled={!poTotalAvailable} onClick={() => onAction(request.id, 'acknowledge')}><Check size={16} />Record acknowledgement</button> : null}</div><div className="movement-history-title po-activity-title"><strong>Purchase Order activity</strong><span>Latest first</span></div><div className="movement-timeline po-activity-timeline">{poActivity.length ? poActivity.map((event, index) => <div className="movement-event" key={`${event.createdAt}-${index}`}><span><Check size={13} /></span><div><b>{movementActionLabel(event.action)}</b><p>{event.detail}</p><small>{event.actor} · {formatMovementTime(event.createdAt)}</small></div></div>) : <div className="table-empty">PO movements will appear here.</div>}</div></section></div>{documentRequest ? <PurchaseOrderDocument request={documentRequest} onClose={() => setDocumentRequest(null)} /> : null}</div>;
}
...
function PurchaseOrderDocument({ request, onClose }: { request: PurchaseRequest; onClose: () => void }) {
  ...
  if (!selectedQuote || poTotal === null) return null;
  return <div className="proc-modal-backdrop po-document-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section className="proc-modal po-document" role="dialog" aria-modal="true" aria-labelledby="po-document-title"><header className="modal-header no-print"><div><span className="eyebrow">Purchase order document</span><h2 id="po-document-title">{poNumber}</h2></div><button className="modal-close" type="button" onClick={onClose} aria-label="Close PO document"><X size={19} /></button></header><div className="po-document-sheet">
    ...
    <div className="lci-po-content">
      <PoDocumentSection title="Supplier Information"><div className="lci-po-fields two"><PoDocumentField label="Supplier / Vendor Name" value={request.vendorName || 'Vendor not selected'} /><PoDocumentField label="Contact Person" value={vendor?.contactPerson || 'Not recorded'} /><PoDocumentField className="wide" label="Address" value={vendorAddress} /><PoDocumentField label="Tel. / Mobile No." value={vendor?.phone || 'Contact number on vendor profile'} /><PoDocumentField label="Email Address" value={request.vendorEmail || vendor?.email || 'No vendor email recorded'} /></div></PoDocumentSection>
      ...
      <PoDocumentSection title="Items / Services Ordered"><div className="lci-po-items"><div className="lci-po-item-head"><b>#</b><b>Qty</b><b>UOM</b><b>Product Name</b><b>Description / Specs</b><b>Unit Price</b><b>Discount</b><b>Amount</b></div>{request.items?.map((item, index) => { const unitPrice = selectedQuote.items.find((quotedItem) => quotedItem.name === item.name)?.unitPrice ?? 0; return <div className="lci-po-item-row" key={`${item.name}-${index}`}><span>{index + 1}</span><span>{item.quantity}</span><span>{itemUom(item)}</span><strong>{item.name}</strong><span>{itemDescription(item)}</span><span>{money.format(unitPrice)}</span><span>{money.format(0)}</span><b>{money.format(item.quantity * unitPrice)}</b></div>; })}</div><div className="lci-po-totals"><span>Subtotal</span><strong>{money.format(poTotal)}</strong><b>Total</b><em>{money.format(poTotal)}</em></div></PoDocumentSection>
      ...
      <PoDocumentSection title="Approvals & Signatures"><div className="lci-po-signatures"><PoSignature role="Processed By" name="Procurement Officer" detail="Admin / Procurement Office" complete /><PoSignature role="Budget Under" name={request.department} detail="Department allocation" complete /><PoSignature role="Requested By" name={request.requester} detail="Requestor" complete />{requestIncludesTechnology(request) ? <PoSignature role="DT Technical Approval" name={request.dtReviewedBy || 'Pending approval'} detail={request.dtReviewedAt ? `Electronically approved on ${formatMovementTime(request.dtReviewedAt)}` : 'DT technical review'} complete={Boolean(request.dtReviewedBy && request.dtReviewedAt)} /> : null}<PoSignature role="Reviewed By" name={departmentApproved ? 'Department Head' : 'Pending approval'} detail="Department Head approval" complete={departmentApproved} /><PoSignature role="Approved By" name={executiveApproved ? executiveNotificationRole(poTotal) : 'Pending approval'} detail="Amount-based executive approval" complete={executiveApproved} /></div></PoDocumentSection>
    </div>
...
function ReceivingQueueView({ requests, role, onAction, onRequestSaved }: { requests: PurchaseRequest[]; role: Role; onAction: (id: string, action: string) => void; onRequestSaved: (request: PurchaseRequest) => void }) {
  const [selectedId, setSelectedId] = useState(requests[0]?.id ?? '');
  const request = requests.find((item) => item.id === selectedId) ?? requests[0];
  ...
  return <div className="proc-page"><PageHeading eyebrow="Delivery and acceptance" title="Receiving" detail="Inspect deliveries, attach invoices, and monitor APS closure." /><div className="approval-layout receiving-master-detail"><section className="proc-card queue-card"><CardHeader title={`Deliveries (${requests.length})`} icon={Truck} />{requests.map((item) => <button type="button" className={`queue-item ${request.id === item.id ? 'selected' : ''}`} key={item.id} onClick={() => setSelectedId(item.id)}><span className="queue-badge"><PackageCheck size={16} /></span><span><b>{item.title}</b><small>{item.id} · {item.status}</small></span><strong>{purchaseOrderTotalLabel(item)}</strong></button>)}</section><ReceivingView key={request.id} request={request} role={role} onAction={onAction} onRequestSaved={onRequestSaved} /></div></div>;
}

function ReceivingView({ request, role, onAction, onRequestSaved }: { request?: PurchaseRequest; role: Role; onAction: (id: string, action: string) => void; onRequestSaved: (request: PurchaseRequest) => void }) {
  ...
  const canAttach = canReceive || role === 'Finance Manager';
  return <div className="proc-page">
    <PageHeading eyebrow="Delivery and acceptance" title="Receiving" detail="Review delivery, acceptance, invoices, and APS payment status." />
    <section className="proc-card receive-card">
      <div className="receive-top"><span className="receive-icon"><PackageCheck size={24} /></span><div><StatusBadge>{request.status}</StatusBadge><h3>{purchaseOrderNumber(request)} / {request.title}</h3><p>{request.department} / {request.category}</p></div></div>
      <ApprovalItems request={request} />
      <div className="policy-checks"><CheckRow done={received} label="Delivery received and accepted" /><CheckRow done={['Paid', 'Filed'].includes(request.status)} label="Payment confirmed" /><CheckRow done={request.status === 'Filed'} label="Closed and filed" /></div>
      <h3>Invoice attachments</h3>
      <RecordAttachments value={request.invoices ?? []} kinds={['Vendor invoice']} onBusyChange={setUploading} onChange={canAttach && !['Paid', 'Filed'].includes(request.status) ? (invoices) => { const createdAt = new Date().toISOString(); onRequestSaved({ ...request, invoices, updatedAt: createdAt, history: [...(request.history ?? []), { action: 'invoice_updated', actor: role, detail: 'Invoice attachments updated for delivery and acceptance.', createdAt }] }); } : undefined} />
      {request.status === 'Received' ? <div className="exception-note"><CircleDollarSign size={18} /><div><b>Awaiting APS admin closure</b><small>The Automated Payment System admin closes this order in APS. The APS connection is pending.</small></div></div> : null}
      <div className="action-row">{!received && canReceive ? <button className="proc-primary" disabled={uploading || purchaseOrderTotal(request) === null} onClick={() => onAction(request.id, 'receive')}><CheckCircle2 size={17} />Record receipt</button> : null}</div>
    </section>
  </div>;
}
...
function VendorInformationFormPage({ vendor, onClose }: { vendor: VendorRecord; onClose: () => void }) {
  ...
  const [submitted, setSubmitted] = useState(false);
  const [confirmed, setConfirmed] = useState(false);
  const [documents, setDocuments] = useState<RecordAttachment[]>(vendor.businessDocuments ?? []);
  ...
  const save = () => { try { const records = loadVendorRecords(); if (!records.some((item) => item.email === vendor.email)) throw new Error('Vendor record no longer exists.'); const updated = { ...form, businessDocuments: documents, informationStatus: 'Information complete' as const }; window.localStorage.setItem('procurement-vendors', JSON.stringify(records.map((item) => item.email === vendor.email ? updated : item))); setSubmitted(true); } catch { setSaveError('Unable to save vendor information. Please try again.'); } };
  ...
  return <div className="proc-modal-backdrop"><section className="proc-modal vendor-stepper vendor-onboarding-stepper" role="dialog" aria-modal="true" aria-labelledby="vendor-onboarding-form-title">
    ...
    <div className="vendor-step-content">
      ...
      {step===3?<section className="vendor-step-panel"><header><div><h3>Payment and compliance</h3><p>Add settlement details and supporting documents.</p></div><strong>Step 3 of 3</strong></header><div className="vendor-commercial-grid"><label><span>Payment terms</span><select value={form.terms} onChange={e=>update('terms',e.target.value)}><option>Immediate</option><option>15 days</option><option>30 days</option><option>45 days</option></select></label><label><span>Bank name</span><input value={form.bankName} onChange={e=>update('bankName',e.target.value)}/></label><label><span>Account name</span><input value={form.bankAccountName} onChange={e=>update('bankAccountName',e.target.value)}/></label><label><span>Account number</span><input value={form.bankAccountNumber} onChange={e=>update('bankAccountNumber',e.target.value)}/></label><div className="span-2"><h4>Business documents</h4><RecordAttachments value={documents} onChange={setDocuments} onBusyChange={setUploading} kinds={['Business registration (SEC / DTI)', 'Tax registration certificate', 'Business permit', 'Other business documents']} /></div></div><label className="vendor-confirmation"><input type="checkbox" checked={confirmed} onChange={e=>setConfirmed(e.target.checked)}/><span><b>I confirm this information is complete and authorized.</b><small>Procurement will verify the submission before updating the vendor record.</small></span></label></section>:null}
    </div>{saveError ? <p role="alert">{saveError}</p> : null}<footer className="vendor-step-footer"><span>Step {step} of 3</span><div>{step>1?<button className="proc-secondary" disabled={uploading} onClick={()=>setStep(step-1)}><ArrowLeft size={16}/>Back</button>:null}{step<3?<button className="proc-primary" disabled={step===1?!firstReady:!secondReady} onClick={()=>setStep(step+1)}>Continue<ArrowRight size={16}/></button>:<button className="proc-primary" disabled={!confirmed || uploading} onClick={save}><Send size={16}/>Submit vendor information</button>}</div></footer>
  </section></div>;
}
...
function VendorDetailPage({ vendor, requests, onBack, onEdit, onRequestInformation, editor }: { vendor: VendorRecord; requests: PurchaseRequest[]; onBack: () => void; onEdit: () => void; onRequestInformation?: () => void; editor: React.ReactNode }) {
  const total = requests.reduce((sum, request) => sum + (purchaseOrderTotal(request) ?? 0), 0);
  ...
  return <div className="proc-page vendor-profile-page"><PageHeading eyebrow="Vendor profile" title={vendor.name} detail={`${vendor.vendorType || 'Company'} · ${vendor.email}`} action={<div className="vendor-profile-actions"><button className="proc-secondary" onClick={onBack}><ArrowLeft size={16} />Back to vendors</button>{onRequestInformation ? <button className="vendor-info-request-button" onClick={onRequestInformation}><Mail size={16} />{informationStatus === 'Invitation pending' ? 'Resend information request' : 'Request information'}</button> : null}<button className="proc-primary" onClick={onEdit}>Edit vendor</button></div>} /><section className="proc-card vendor-profile-card"><div className="vendor-profile-hero"><span className="vendor-profile-avatar">{vendor.name.charAt(0).toUpperCase()}</span><div><div className="vendor-profile-badges"><StatusBadge>{vendor.vendorType || 'Company'}</StatusBadge><VendorInformationBadge status={informationStatus} /></div><h2>{vendor.name}</h2><p>{vendor.email}{vendor.phone ? ` · ${vendor.phone}` : ''}</p></div></div><div className="vendor-profile-section"><h3>Contact and address</h3><div className="vendor-profile-grid"><Detail label="Email" value={vendor.email} /><Detail label="Contact person" value={vendor.contactPerson || 'Not recorded'} /><Detail label="Contact number" value={vendor.phone || 'Not recorded'} /><Detail label="Address" value={address} /><Detail label="Website" value={vendor.website || 'Not recorded'} /></div></div><div className="vendor-profile-section"><h3>Business information</h3><div className="vendor-profile-grid"><Detail label="Tax ID / TIN" value={vendor.taxId || 'Not recorded'} /><Detail label="Branch code" value={vendor.branchCode || '000'} /><Detail label="Tags" value={vendor.tags || 'None'} /><Detail label="Vendor rating" value={vendor.rating} /></div></div><div className="vendor-profile-section"><h3>Information request</h3><div className="vendor-profile-grid"><Detail label="Status" value={informationStatus} /><Detail label="Last requested" value={vendor.informationRequestedAt ? formatMovementTime(vendor.informationRequestedAt) : 'Not yet requested'} /><Detail label="Recipient" value={vendor.email} /><Detail label="Managed by" value="Procurement Office" /></div></div><div className="vendor-profile-section"><h3>Purchasing settings</h3><div className="vendor-profile-grid"><Detail label="Payment terms" value={vendor.terms} /><Detail label="Lead time" value={vendor.lead} /><Detail label="Total purchases" value={String(requests.length)} /><Detail label="Total purchase value" value={money.format(total)} /></div></div><div className="vendor-profile-section"><h3>Business documents</h3><RecordAttachments value={vendor.businessDocuments ?? []} kinds={[]} />{vendor.complianceDocuments?.length ? <p>Previously recorded filenames: {vendor.complianceDocuments.join(', ')}</p> : null}</div><div className="vendor-profile-section"><h3>Notes</h3><p className="vendor-profile-notes">{vendor.notes || 'No internal notes recorded.'}</p></div><div className="vendor-profile-section"><div className="movement-history-title"><strong>Purchase history</strong><span>{requests.length} records</span></div><div className="vendor-purchase-list"><div className="vendor-purchase-head"><span>Purchase order</span><span>Purchase</span><span>Status</span><span>Amount</span></div>{requests.map((request) => <div className="vendor-purchase-row" key={request.id}><span><b>{request.id.replace('PR-', 'PO-')}</b><small>From {request.id}</small></span><span><b>{request.title}</b><small>{request.department}</small></span><StatusBadge>{request.status}</StatusBadge><strong>{purchaseOrderTotalLabel(request)}</strong></div>)}{requests.length === 0 ? <div className="table-empty">No purchases have been assigned to this vendor yet.</div> : null}</div></div></section>{editor}</div>;
}
...
function VendorEditor({ initial, onCancel, onSave }: { initial: VendorRecord; onCancel: () => void; onSave: (vendor: VendorRecord) => void }) {
  const [form, setForm] = useState<VendorRecord>({ vendorType: 'Company', contactPerson: '', phone: '', street: '', street2: '', city: '', state: '', zip: '', country: 'Philippines', taxId: '', branchCode: '000', website: '', tags: '', notes: '', ...initial });
...
function ReportsView({ requests }: { requests: PurchaseRequest[] }) {
  ...
  const totalSpend = filtered.reduce((sum, item) => sum + (workflowFinancialAmount(item) ?? 0), 0);
  const approvalCount = filtered.filter((item) => approvalStatuses.includes(item.status)).length;
  const openPoValue = filtered.filter((item) => openPoStatuses.includes(item.status)).reduce((sum, item) => sum + (purchaseOrderTotal(item) ?? 0), 0);
  ...
  const departmentSpend = [...new Set(filtered.map((item) => item.department))].map((name) => ({ name, amount: filtered.filter((item) => item.department === name).reduce((sum, item) => sum + (workflowFinancialAmount(item) ?? 0), 0) })).sort((a, b) => b.amount - a.amount);
  const categorySpend = [...new Set(filtered.map((item) => item.category))].map((name) => ({ name, amount: filtered.filter((item) => item.category === name).reduce((sum, item) => sum + (workflowFinancialAmount(item) ?? 0), 0) })).sort((a, b) => b.amount - a.amount);
  const vendorSpend = [...new Set(filtered.filter((item) => item.vendorName).map((item) => item.vendorName as string))].map((name) => ({ name, amount: filtered.filter((item) => item.vendorName === name).reduce((sum, item) => sum + (purchaseOrderTotal(item) ?? 0), 0), orders: filtered.filter((item) => item.vendorName === name && openPoStatuses.includes(item.status)).length })).sort((a, b) => b.amount - a.amount);
  ...
  const exportCsv = () => {
    ...
    const rows = [['Request', 'Title', 'Department', 'Category', 'Status', 'Amount', 'Vendor', 'Due'], ...filtered.map((item) => [item.id, item.title, item.department, item.category, item.status, workflowFinancialAmount(item) ?? 'Pending quotation', item.vendorName || '', item.due])];
...
function NewRequestPage({ onBack, onSubmit }: { onBack: () => void; onSubmit: (request: PurchaseRequest) => boolean | Promise<boolean> }) {
  ...
  const [purposeType, setPurposeType] = useState<'Activity' | 'Event'>('Activity');
  ...
  const addManualRequestItem = () => { if (!itemSearch.trim() || !newItemDescription.trim()) return; setRequestItems((current) => [...current, { id: `PRI-${Date.now()}`, name: itemSearch.trim(), category: newItemCategory, description: newItemDescription.trim(), uom: 'UNIT', quantity: Math.max(1, newItemQuantity), unitPrice: Math.max(0, newItemUnitPrice) }]); setItemSearch(''); setNewItemDescription(''); setNewItemQuantity(1); setNewItemUnitPrice(0); setItemPickerOpen(false); };
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (!title.trim() || !purpose.trim() || amount <= 0) return;
    setSubmitting(true);
    await onSubmit({
    ...
    <form className="request-editor" onSubmit={submit}>
      ...
      <section className="request-editor-section">
        ...
        <div className="pr-item-table">
          ...
          <div className="pr-item-picker"><label><Search size={16} /><input value={itemSearch} onFocus={() => setItemPickerOpen(true)} onChange={(event) => { setItemSearch(event.target.value); setItemPickerOpen(true); }} placeholder="Search or add a product" /></label><input aria-label="New product description or specifications" value={newItemDescription} onChange={(event) => setNewItemDescription(event.target.value)} placeholder="Enter description, dimensions, materials, compatibility, or technical specs" /><select aria-label="New product category" value={newItemCategory} onChange={(event) => setNewItemCategory(event.target.value)}><option>Operational supplies</option><option>Technology</option><option>Furniture</option><option>Capital equipment</option><option>Appliance</option><option>Customized item</option><option>Bulk purchase</option><option>Warrantied item</option><option>Services</option></select><span className="pr-picker-uom">UNIT</span><input aria-label="New product quantity" type="number" min="1" value={newItemQuantity} onChange={(event) => setNewItemQuantity(Number(event.target.value))} /><input aria-label="New product estimated unit cost" type="number" min="0" value={newItemUnitPrice} onChange={(event) => setNewItemUnitPrice(Number(event.target.value))} /><strong>{money.format(Math.max(1, newItemQuantity) * Math.max(0, newItemUnitPrice))}</strong>{itemPickerOpen && matchingItems.length > 0 ? <div className="pr-item-results">{matchingItems.map((product) => <button type="button" key={product.name} onClick={() => addRequestItem(product)}><span><b>{product.name}</b><small>{product.category} · {product.uom}</small><small>{product.description}</small></span><strong>{money.format(product.price)}</strong></button>)}</div> : null}</div>
          <div className="pr-item-add-row"><button type="button" onClick={addManualRequestItem} disabled={!itemSearch.trim() || !newItemDescription.trim()}><Plus size={16} />Add product to request</button></div>
        </div>
      ...
      <footer className="request-editor-actions"><button className="proc-secondary" type="button" onClick={onBack}>Cancel</button><button className="proc-primary" type="submit" disabled={submitting || !title.trim() || !purpose.trim() || amount <= 0}><Send size={16} />{submitting ? 'Submitting...' : 'Submit request'}</button></footer>
    </form>
  </div>;
}
...
function UserAdministrationView() {
  ...
  if (!selected) return <EmptyWorkflow title="Users, Roles & Departments" detail="No user assignments have been added." />;
...
function purchaseOrderActivity(request: PurchaseRequest) {
  ...
  const currentStage = ['Paid', 'Filed'].includes(request.status) ? 7 : request.status === 'Received' ? 6 : purchaseOrderStageIndex(request.status);
  const poNumber = purchaseOrderNumber(request);
  const poTotal = purchaseOrderTotal(request);
  ...
  const definitions = [
    ...
    { action: 'receive', actor: 'receiving@life.edu.ph', detail: `Delivery for ${poNumber} received and accepted. Awaiting closure by the Automated Payment System admin.` },
  ...
  const activity: Movement[] = definitions.slice(0, currentStage + 1).map((definition, stage) => {
    ...
    return { ...definition, ...movement, createdAt: movement?.createdAt ?? stageTime(stage), actor: movement?.actor || definition.actor };
  });
  if (request.status === 'Filed') {
    const filed = source.find((event) => event.action === 'file');
    activity.push({ action: 'file', actor: filed?.actor || 'Recorded closure', detail: filed?.detail || `${poNumber} closed and filed with its supporting procurement records.`, createdAt: filed?.createdAt ?? new Date(safeFinish + 60_000).toISOString() });
  }
  activity.push(...source.filter((event) => !activity.some((item) => item.action === event.action && item.createdAt === event.createdAt) && !['approve', 'po_stage', 'file'].includes(event.action)));
...
function assignedTeamForStatus(status: RequestStatus) {
  ...
  if (status === 'For Finance Approval') return 'Finance';
  if (['Received', 'Paid', 'Filed'].includes(status)) return 'Automated Payment System Admin';
  ...
  return 'Requester / Assigned Custodian';
}
...
function isPurchaseOrderStatus(status: RequestStatus) {
  return ['PO Draft', 'For Department Approval', 'For Finance Approval', 'For COO Approval', 'For President Approval', 'PO Approved', 'PO Awaiting Acknowledgement', 'PO Acknowledged', 'Partially Received', 'Received', 'Paid', 'Filed'].includes(status);
}
...
function purchaseOrderStageIndex(status: RequestStatus) {
  ...
  if (['Received', 'Paid'].includes(status)) return 5;
  if (status === 'Filed') return 6;
  return 3;
}
