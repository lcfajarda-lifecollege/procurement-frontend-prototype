...
function lifecycleSeedPo(options: { id: string; title: string; department: string; amount: number; requester: string; status: RequestStatus; itemName: string; category?: string; quantity?: number; vendorName?: string; vendorEmail?: string; createdAt: string; updatedAt: string }): PurchaseRequest {
  ...
  return {
    ...
    rfqQuotes: [lifecycleSeedQuote(vendorName, vendorEmail, 'Responded', `${options.id.replace('PR-', 'RFQ-')}-A`, [{ name: options.itemName, unitPrice: options.amount / quantity }])],
...
const demoRequests: PurchaseRequest[] = [
  ...
  { id:'PR-2026-1022', title:'Administration Workplace Refresh', department:'Administration', amount:152100, category:'Multiple categories', requester:'Lem Fajarda', status:'RFQ Draft', due:'In 11 days', items:[{name:'Laptop Computer',category:'Technology',description:'14-inch business laptop, Core Ultra 7 class, 16 GB RAM, 512 GB SSD',uom:'UNIT',quantity:3,unitPrice:45000},{name:'Printer Ink',category:'Operational supplies',description:'Original high-yield ink cartridge compatible with office printers',uom:'CARTRIDGE',quantity:12,unitPrice:950},{name:'Bond Paper A4',category:'Operational supplies',description:'A4, 80 gsm, 500 sheets per ream',uom:'REAM',quantity:20,unitPrice:285}], createdAt:'2026-08-28T01:00:00Z', updatedAt:'2026-08-28T04:00:00Z', rfqQuotes:[{...lifecycleSeedQuote('Power Mac Center, Inc.','education@powermaccenter.com','Draft','RFQ-2026-1022-TECH-A',[{name:'Laptop Computer',unitPrice:44200}],9),lotCategories:['Technology']},{...lifecycleSeedQuote('Enterprise Technology Solutions','bids@ets.example','Draft','RFQ-2026-1022-TECH-B',[{name:'Laptop Computer',unitPrice:45100}],7),lotCategories:['Technology']},{...lifecycleSeedQuote('Office Warehouse, Inc.','bids@officewarehouse.example','Draft','RFQ-2026-1022-OPS-A',[{name:'Printer Ink',unitPrice:925},{name:'Bond Paper A4',unitPrice:278}],5),lotCategories:['Operational supplies']},{...lifecycleSeedQuote('Lem Fajarda Merchandise','sales@lemfajarda.example','Draft','RFQ-2026-1022-OPS-B',[{name:'Printer Ink',unitPrice:940},{name:'Bond Paper A4',unitPrice:282}],6),lotCategories:['Operational supplies']}], history:[{action:'create',actor:'lem.fajarda@life.edu.ph',detail:'Department Head and COO notified based on the ₱152,100 request total. Routed to Procurement Review.',createdAt:'2026-08-28T01:00:00Z'},{action:'complete_review',actor:'procurement@life.edu.ph',detail:'Procurement Review separated the request into Technology and Operational supplies sourcing lots.',createdAt:'2026-08-28T03:00:00Z'},{action:'add_qualified_vendor',actor:'procurement@life.edu.ph',detail:'Two qualified technology vendors added to the Technology sourcing lot.',createdAt:'2026-08-28T03:30:00Z'},{action:'add_qualified_vendor',actor:'procurement@life.edu.ph',detail:'Two qualified office-supplies vendors added to the Operational supplies sourcing lot.',createdAt:'2026-08-28T04:00:00Z'}] },
...
const procurementDataVersion = 'full-pr-po-lifecycle-seed-2026-08-28-v13';
const twoLotSampleVersion = 'two-lot-rfq-draft-v2';
...
function loadRequestRecords(): PurchaseRequest[] {
  try {
    if (window.localStorage.getItem('procurement-data-version') !== procurementDataVersion) {
      ...
      window.localStorage.setItem('procurement-two-lot-sample-version', twoLotSampleVersion);
    ...
    const requiredSample = demoRequests.find((request) => request.id === 'PR-2026-1022');
    const sampleNeedsRefresh = window.localStorage.getItem('procurement-two-lot-sample-version') !== twoLotSampleVersion;
    const merged = requiredSample && sampleNeedsRefresh ? [requiredSample, ...records.filter((request) => request.id !== requiredSample.id)] : requiredSample && !records.some((request) => request.id === requiredSample.id) ? [requiredSample, ...records] : records;
    if (sampleNeedsRefresh) window.localStorage.setItem('procurement-two-lot-sample-version', twoLotSampleVersion);
    return hydrateRequestItems(merged);
  } catch {
    return hydrateRequestItems(demoRequests);
  }
}
...
function ProcurementReview({ request, onBack, onComplete, onNotify }: { request: PurchaseRequest; onBack: () => void; onComplete: () => void; onNotify: (message: string) => void }) {
  const [notes, setNotes] = useState('');
  return <div className="proc-page"><PageHeading eyebrow="Procurement Officer workspace" title="Procurement Review" detail="Validate the submitted Purchase Request before beginning vendor sourcing." action={<button className="proc-secondary" type="button" onClick={onBack}><ArrowLeft size={16} />Back to all RFQs &amp; Sourcing</button>} /><div className="approval-layout procurement-review-layout"><section className="proc-card queue-card"><CardHeader title="For procurement review" icon={Inbox} /><button type="button" className="queue-item selected"><span className="queue-badge">{request.requester.split(' ').map((part) => part[0]).join('').slice(0,2)}</span><span><b>{request.title}</b><small>{request.id} · Stakeholders notified</small></span></button></section><section className="proc-card review-card"><div className="review-header"><div><StatusBadge>{request.status}</StatusBadge><h3>{request.title}</h3><p>{request.id} · Requested by {request.requester}</p></div></div><div className="detail-grid"><Detail label="Department" value={request.department} /><Detail label="Required date" value={request.due} /><Detail label="Product lines" value={String(request.items?.length ?? 0)} /><Detail label="Total quantity" value={String(requestTotalQuantity(request))} /></div><div className="movement-history-title"><strong>Requested products</strong><span>Review specifications and quantities</span></div><div className="request-detail-items procurement-review-items">{request.items?.map((item, index) => <div key={`${item.name}-${index}`}><span><b>{item.name}</b><small>{item.category} · {itemUom(item)}</small><em>{itemDescription(item)}</em></span><span>{item.quantity} {itemUom(item)}</span></div>)}</div><label className="notes-field procurement-review-notes"><span>Procurement review notes</span><textarea value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Record clarifications, sourcing instructions, or vendor requirements" /></label><div className="action-row"><button className="proc-secondary" disabled={!notes.trim()} onClick={() => onNotify(`${request.id} returned to requester for clarification: ${notes.trim()}`)}>Return for clarification</button><button className="proc-primary" onClick={onComplete}><Check size={17} />Complete review and begin sourcing</button></div></section></div></div>;
}
