...
function procurementOfficerStagePreviews() {
  const source = demoRequests.find((request) => request.id === 'PR-2026-1001');
  if (!source) return [];
  const respondedQuotes: RfqVendorQuote[] = [
    { vendorName: 'Power Mac Center, Inc.', vendorEmail: 'education@powermaccenter.com', status: 'Responded', reference: 'RFQ-2026-1001-A', deliveryDays: 12, terms: '30 days', warranty: 'One year onsite warranty', validUntil: '2026-09-30', attachmentName: 'RFQ-2026-1001-A.pdf', lotCategories: ['Technology'], items: [{ name: 'Projector', unitPrice: 31500 }, { name: 'Laptop Computer', unitPrice: 44200 }] },
    { vendorName: 'Enterprise Technology Solutions', vendorEmail: 'bids@ets.example', status: 'Responded', reference: 'RFQ-2026-1001-B', deliveryDays: 10, terms: '30 days', warranty: 'One year parts and labor', validUntil: '2026-09-30', attachmentName: 'RFQ-2026-1001-B.pdf', lotCategories: ['Technology'], items: [{ name: 'Projector', unitPrice: 30900 }, { name: 'Laptop Computer', unitPrice: 45300 }] },
  ];
  const stages: RequestStatus[] = ['For Procurement Review', 'RFQ Draft', 'RFQ Sent', 'Quotations Received', 'For DT Approval', 'For Requester Selection', 'Ready for PO Creation'];
  return stages.map((status, index): PurchaseRequest => {
    const quotes = status === 'For Procurement Review' ? [] : respondedQuotes.map((quote) => ({
      ...quote,
      status: status === 'RFQ Draft' ? 'Draft' as const : status === 'RFQ Sent' ? 'Sent' as const : 'Responded' as const,
      reference: status === 'RFQ Draft' ? '' : quote.reference,
      attachmentName: ['For Procurement Review', 'RFQ Draft', 'RFQ Sent'].includes(status) ? undefined : quote.attachmentName,
    }));
    const reviewed = ['For Requester Selection', 'Ready for PO Creation'].includes(status);
    const ready = status === 'Ready for PO Creation';
    return {
      ...source,
      id: `${source.id}-PREVIEW-${index + 1}`,
      sourceRequestId: source.id,
      status,
      rfqQuotes: quotes,
      procurementValidationNotes: ['For DT Approval', 'For Requester Selection', 'Ready for PO Creation'].includes(status) ? 'Both quotations are complete, commercially comparable, and ready for the next review.' : undefined,
      procurementValidatedAt: ['For DT Approval', 'For Requester Selection', 'Ready for PO Creation'].includes(status) ? '2026-09-03T05:00:00Z' : undefined,
      procurementValidatedBy: ['For DT Approval', 'For Requester Selection', 'Ready for PO Creation'].includes(status) ? 'Procurement Office' : undefined,
      dtReviewNotes: reviewed ? 'Both vendors meet the required projector and laptop specifications.' : undefined,
      dtReviewedAt: reviewed ? '2026-09-03T07:00:00Z' : undefined,
      dtReviewedBy: reviewed ? 'Digital Transformation Team' : undefined,
      sourcingAwards: ready ? [{ category: 'Technology', vendorName: respondedQuotes[0].vendorName, vendorEmail: respondedQuotes[0].vendorEmail, quoteReference: respondedQuotes[0].reference }] : undefined,
      vendorName: ready ? respondedQuotes[0].vendorName : undefined,
      vendorEmail: ready ? respondedQuotes[0].vendorEmail : undefined,
      demoStagePreview: true,
...
function loadRequestRecords(): PurchaseRequest[] {
  try {
    ...
    if (!saved) return hydrateRequestItems(demoRequests);
    const records = JSON.parse(saved) as PurchaseRequest[];
    return Array.isArray(records) && records.length ? hydrateRequestItems(records) : hydrateRequestItems(demoRequests);
  } catch {
    return hydrateRequestItems(demoRequests);
  }
}
...
function MultiSourcingView({ requests, showLifecyclePreviews, onAction, onVendorSaved, onNotify }: { requests: PurchaseRequest[]; showLifecyclePreviews: boolean; onAction: (id: string, action: string) => void; onVendorSaved: (request: PurchaseRequest) => void; onNotify: (message: string) => void }) {
  ...
  const lifecyclePreviews = showLifecyclePreviews ? procurementOfficerStagePreviews().filter((preview) => preview.status !== liveSource?.status) : [];
  const displayRequests = showLifecyclePreviews ? [...(liveSource ? [liveSource] : []), ...lifecyclePreviews, ...requests.filter((request) => request.id !== liveSource?.id)] : requests;
...
function PurchaseOrderDocument({ request, onClose }: { request: PurchaseRequest; onClose: () => void }) {
  ...
  return <div className="proc-modal-backdrop po-document-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section className="proc-modal po-document" role="dialog" aria-modal="true" aria-labelledby="po-document-title"><header className="modal-header no-print"><div><span className="eyebrow">Purchase order document</span><h2 id="po-document-title">{poNumber}</h2></div><button className="modal-close" type="button" onClick={onClose} aria-label="Close PO document"><X size={19} /></button></header><div className="po-document-sheet">
    ...
    <div className="lci-po-content">
      ...
      <PoDocumentSection title="Approvals & Signatures"><div className="lci-po-signatures"><PoSignature role="Processed By" name="Procurement Officer" detail="Admin / Procurement Office" complete /><PoSignature role="Budget Under" name={request.department} detail="Department allocation" complete /><PoSignature role="Requested By" name={request.requester} detail="Requestor" complete /><PoSignature role="Reviewed By" name={departmentApproved ? 'Department Head' : 'Pending approval'} detail="Department Head approval" complete={departmentApproved} /><PoSignature role="Approved By" name={executiveApproved ? executiveNotificationRole(poTotal) : 'Pending approval'} detail="Amount-based executive approval" complete={executiveApproved} /></div></PoDocumentSection>
    </div>
